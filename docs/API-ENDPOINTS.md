...
- `GET /api/v1/me` — Returns the authenticated user record (Mongo) and related parent/guardian profile (with populated children). Requires Clerk session/JWT. Returns 404 if user doesn't exist (no auto-creation to prevent incorrect role assignment).
...
- `PUT /api/v1/me/student-profile` — Updates the authenticated student's profile. For invited students without a User record, automatically creates one with `role: 'student'` by matching Clerk email to Student.invitedEmail, links the records, and sets `invitationStatus: 'accepted'`. Accepts `displayName`, `yearGroup`, and `enrolments` array.
...
- Body:
  ...
  {
    "yearGroup": "Year 6",
    "enrolments": [{ "subject": "Physics", "examBody": "AQA" }]
  }
  ```
